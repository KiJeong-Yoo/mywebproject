...
<!-- 검색리스트 -->
<form name="form" action="/photoboard/searchList" method="post">
...
<tr onmouseover="this.style.backgroundColor='lightgray'" 
	...
	onclick="location.href='/photoboard/read?idx=${board.idx}&requestPage=${pageboard.requestPage}'">
<td>${board.idx}</td>
<td>
...
<ul class="pagination">
    ...
    <li class="page-item"><a class="page-link" href="/photoboard/photomain?requestPage=${pageboard.beginPage - 5}">이전페이지</a></li>
    </c:if>
 
    <c:forEach var="i" begin="${pageboard.beginPage}" end="${pageboard.endPage}">
    <li class="page-item"><a class="page-link" href="/photoboard/photomain?requestPage=${i}">${i}</a></li>
    </c:forEach>
   
    <c:if test="${pageboard.totalPage ne pageboard.endPage}">
    <li class="page-item"><a class="page-link" href="/photoboard/photomain?requestPage=${pageboard.endPage + 1}">다음페이지</a></li>
    </c:if>
    
  </ul>
</td>
<td>
<button onclick="location.href='/photoboard/insert'">글쓰기</button>
</td>
</tr>
<!-- end page list -->
</table>
